...
class AuthController {
  ...
  async login(req, res, next) {
    ...
    try {
      ...
      // hash the password
      const verifyHashedPassword = await passwordServices.verifyPass(
        password,
        user.password
      );
      ...
      await tokenServices.storeRefreshToken({
        token: refreshToken,
        userId: user._id,
      });
  ...
  async refresh(req, res, next) {
    try {
      ...
      // verify token
      const verifyToken = tokenServices.verifyRefreshToken(
      ...
      const tokenInDb = await tokenServices.findRefreshToken({
      ...
      if (!tokenInDb) {
        return res
          .status(401)
          .json(CustomErrorHandler.unAuthorized("Invalid Token!"));
      }
      ...
      if (!user) {
        return res
          .status(401)
          .json(CustomErrorHandler.notFound("User not found!"));
      }
      // create a token
      const { refreshToken, accessToken } = tokenServices.generateToken({
        id: user._id,
